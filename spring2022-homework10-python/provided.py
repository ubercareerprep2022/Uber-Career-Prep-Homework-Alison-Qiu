...
def bigram_viterbi(hmm, sentence):
    ...
    for i in range(1, len(sentence)):
        backpointer["No_Path"][i] = "No_Path"
        for l in unique_tags:
            max_value = -1 * float('inf')
            max_state = None
            for l_prime in unique_tags:
                val1= viterbi[l_prime][i-1]
                val2 = -1 * float('inf')
                if hmm.transition_matrix[l_prime][l] != 0:
                    val2 = math.log(hmm.transition_matrix[l_prime][l])
            ...
            viterbi[l][i] = max_value + val3
            if max_state == None:
                backpointer[l][i] = "No_Path"
            else:
                backpointer[l][i] = max_state
    ...
    for l_prime in unique_tags:
        if viterbi[l_prime][final_time] > max_value:
            max_value = viterbi[l_prime][final_time]
            last_state = l_prime
    if last_state == None:
        last_state = "No_Path"
    ...
    for i in range(len(sentence)-2, -1, -1):
        #tagged_sentence[-1][1] is the state of the most newly appended element
        next_tag = tagged_sentence[-1][1]
        #curr_tag is bp[Zi+1, i+1]
